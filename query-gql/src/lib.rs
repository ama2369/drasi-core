...
peg::parser! {
    grammar gql() for str {
        ...
        rule return_clause() -> Vec<Expression>
            = kw_return() __+ items:( projection_expression() ++ (__* "," __*) ) { 
                items 
            }
        ...
        rule part(config: &dyn GQLConfiguration) -> QueryPart
...
pub fn parse(
    ...
    let result = gql::query(input, config);
    result
}
