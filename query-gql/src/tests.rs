...
struct TestGQLConfig {}

impl GQLConfiguration for TestGQLConfig {
...
static TEST_CONFIG: TestGQLConfig = TestGQLConfig {};

#[test]
fn simple_match_clause() {
    assert_eq!(
        gql::query(
...
fn match_clause() {
    ...
    assert_eq!(
        gql::query(" MATCH () -> (:LABEL_ONLY) RETURN a.test", &TEST_CONFIG),
        Ok(Query {
            parts: vec![QueryPart {
                match_clauses: vec![MatchClause {
...
fn multiple_match_clauses() {
    assert_eq!(
        gql::query("MATCH (t:Thing)-[:AT]->(wh:Warehouse), (i:Item)-[:CONTAINS]->(c:Component) RETURN t.name", &TEST_CONFIG),
        Ok(Query {
            parts: vec![QueryPart {
                match_clauses: vec![
                  MatchClause {
...
fn optional_match_clauses() {
    assert_eq!(
        gql::query("MATCH (t:Thing)-[:AT]->(wh:Warehouse) OPTIONAL MATCH (t)-[:CONTAINS]->(c:Component) RETURN t.name", &TEST_CONFIG),
        Ok(Query {
            parts: vec![QueryPart {
                match_clauses: vec![
                  MatchClause {
...
fn where_clauses() {
    assert_eq!(
      gql::query("MATCH (a:Person) -[e:KNOWS]-> (b:Person) WHERE a.age > 42 AND b.name = 'Peter Parker' OR NOT e.fake RETURN e.since", &TEST_CONFIG),
        Ok(Query {
            parts: vec![QueryPart {
                match_clauses: vec![
                  MatchClause {
...
fn with_clauses_non_aggregating() {
    assert_eq!(
        gql::query("MATCH (t:Thing)-[:AT]->(wh:Warehouse) WHERE t.category = 1 WITH wh.name, t RETURN t.name", &TEST_CONFIG),
        Ok(Query {
            parts: vec![QueryPart {
                match_clauses: vec![MatchClause {
                    start: NodeMatch::with_annotation(Annotation::new("t".into()), "Thing".into()),
...
fn where_clause_non_aggregating_with_date_yyyy_mm_dd() {
    let query =
        gql::query("MATCH (a) WHERE a.Field1 = date('2023-09-05') RETURN a.name, a.Field2 as F2, a.Field1, $param", &TEST_CONFIG)
            .unwrap();
...
fn where_clause_non_aggregating_with_date_weeks() {
    let query =
        gql::query("MATCH (a) WHERE a.Field1 = date('2015-W30-2') RETURN a.name, a.Field2 as F2, a.Field1, $param", &TEST_CONFIG)
            .unwrap();
...
fn where_clause_non_aggregating_with_date_yyyy_quarters() {
    let query =
        gql::query("MATCH (a) WHERE a.Field1 = date('2015-Q2-02') RETURN a.name, a.Field2 as F2, a.Field1, $param", &TEST_CONFIG)
            .unwrap();
...
fn where_clause_non_aggregating_with_local_time() {
    let query =
        gql::query("MATCH (a) WHERE a.Field1 = localtime('12:58:04') RETURN a.name, a.Field2 as F2, a.Field1, $param", &TEST_CONFIG)
            .unwrap();
...
fn test_function_empty_args() {
    let query = gql::query("MATCH (a) RETURN datetime()", &TEST_CONFIG).unwrap();
...
fn where_follows_with_no_alias() {
    let query =
        gql::query("MATCH (a) WITH a WHERE 1 = 1 RETURN a.Field1", &TEST_CONFIG).unwrap();
